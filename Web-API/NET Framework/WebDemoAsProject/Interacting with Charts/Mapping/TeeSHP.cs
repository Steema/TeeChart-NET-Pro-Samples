...
namespace WebDemo.Interactive_Charting.TeeMapWeb
{
	...
	public class TeeSHP
	{
		...
		private int po;
		private ListItemCollection debug;
		private string path;

		public TeeSHP(string Path)
		{
			path = Path;
		}
		...
		public void LoadMap(Map Series, string FileName) {
			LoadMap(Series,FileName,null,null,null, null);
		}

		
		public void LoadMap(Map Series, string FileName, DataTable Table, string FieldName, string FieldValue,ListItemCollection Debug) {
			FileStream f, fx;
			...
			string tmpName=ExtractFilePath(FileName, ".shp");
		
			f = File.OpenRead(tmpName);
			...
			try {
				...
				tmpName=ExtractFilePath(FileName, ".shx");
				fx = File.OpenRead(tmpName);
				try {
					fx.Position = 0;
...
//						else
		...
		private string ExtractFilePath(string FileName, string Extension) {
			string tmpFileName;
			int i = FileName.IndexOf(".");
			if(i == -1) 
				tmpFileName = FileName + Extension;
			else {
				tmpFileName = FileName.Remove(i, FileName.Length - i);
